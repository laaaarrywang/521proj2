...
To take the fact that data is not i.i.d. into account, we would like to split the image into several "blocks" that are treated as sub-images, such that each block only borders other blocks at the boundary. This data splitting rule significantly reduces the similarity between training set and validation set as well as the testing set. In the meantime, we want each sub-image contains fair amount of clouds and surfaces to achieve better performances. Here, we introduce two approaches to create the blocks.

The first one is imspired by the nature of K-means clustering. 

```{r}
km.out1.bad <- kmeans(image1%>%filter(label!=0)%>%.[c(4,5,6)],5) # bad result
km.out1.good <- kmeans(image1%>%filter(label!=0)%>%.[c(1,2)],5)
km.out2 <- kmeans(image2[-c(1,2,3)],5)
km.out3 <- kmeans(image3[-c(1,2,3)],5)
```

```{r}
image1 %>%
  filter(label!=0) %>%
  mutate(class=km.out1.good$cluster) %>%
  ggplot(aes(x,y)) +
