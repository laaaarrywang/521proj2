...
CVmaster = function(classifier, features, labels, K = 5, loss_method = NULL, split_method, coordinate){
  ...
  for(k in 1:K){
    ...
    # training
    if(classifier == "LDA"){
      LDA = lda(labels.train ~ features.train)
      predict_LDA = predict(LDA, features.validate)
      loss = c(loss, mean(predict_LDA == labels.validate))
    }
    else if(classifier == "QDA"){
      QDA = qda(labels.train ~ features.train)
      predict_QDA = predict(QDA, features.validate)
      loss = c(loss, mean(predict_QDA == labels.validate))
    }
    
    # fit = classifier(features.train, labels.train, ...)
    # 
    # # predicting
    # labels.predicted = something(features.validate, fit) ### TBD by the exact function
    # 
    # loss = c(loss, loss_method(labels.validate, labels.predicted))
